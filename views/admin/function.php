...
{
  global $conn; // Sử dụng biến $conn được khai báo từ file connect.php

  if (isset($_GET['key'])) {
    $idkey = mysqli_real_escape_string($conn, $_GET['key']);

    $sql_quyenhan = "SELECT * FROM capquyen where user='$idkey'";
    $result_thanhvien = mysqli_query($conn, $sql_quyenhan);
...
{
  // Viết code để xử lý chuỗi $str và mã hóa các ký tự đặc biệt
  // Ví dụ: 
  // $encodedStr = some_encode_function($str);
  // return $encodedStr;

  // Đây là một ví dụ đơn giản để mã hóa các ký tự đặc biệt bằng htmlentities
