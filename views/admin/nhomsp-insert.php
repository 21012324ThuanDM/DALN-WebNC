...
if (isset($_POST["act"])) {
	$ten = $_POST["ten"];
	$id = getid(); // Đảm bảo rằng hàm getid đã được định nghĩa và sử dụng kết nối mysqli $conn
	...
	if ($n != 0)
		...
		if (!$kq)
			...
			echo "<script>alert('Đã thêm');window.location='../admincp/ql-nhom-loai.php?m=mn&b=nhomsp-list'</script>";
			$ten = "";
		}
	}
}
