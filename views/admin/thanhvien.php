<?php
if (isset($_GET['active']) && isset($_GET['n'])) {
	$keyactive = $_GET["active"];
	...
	if ($keyactive == "administrator" || $keyactive == "admin")
		...
		if ($number == 1) {
			$result_hieuluc = mysqli_query($connection, "UPDATE thanhvien SET hieuluc=0 WHERE user='$keyactive'");
		}
		if ($number == 0) {
			$result_hieuluc = mysqli_query($connection, "UPDATE thanhvien SET hieuluc=1 WHERE user='$keyactive'");
		}
	}
	if ($result_hieuluc) {
		echo "<script>window.location='?m=thanhvien';</script>";
	}
}
if (isset($_POST["act"])) {
	if (isset($_POST["xoa"])) {
		...
		if ($count == 0) {
			...
			$n = 0;
			foreach ($chon as $user) {
				if ($user == "administrator" || $user == "admin") {
					echo "<script>alert('Không thể xóa tài khoản quản trị');</script>";
				} else {
					$SQL_xoagiohang = "DELETE FROM thanhvien WHERE user='$user'";
					// echo "$SQL_xoagiohang";
					$kq_xoagiohang = mysqli_query($connection, $SQL_xoagiohang);
...
<form method="post">
	<table width="960" height="70" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF"
		bordercolordark="#FFFFFF">
		<tr>
			<td style="border-top:1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC; border-left:1px solid #CCCCCC">
				<div align="left"
					style="color:#d4340c; font-family:Tahoma; font-size: 16px; font-weight:bold; padding-left:20px">QUẢN
					LÝ THÀNH VIÊN</div>
			</td>
			<td
				style="border-top:1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC; border-right:1px solid #CCCCCC; height:65px; width:55px">
				<input type="submit" name="xoa" value='' onClick="return checktv();document.form.submit();"
					style="background:url(../images/bt_xoa.jpg); width:55px; height:65px;">
			</td>
		</tr>
	</table>
	...
	<table width="960" border="0" cellspacing="0" cellpadding="0">
		...
		<tr height="30" bgcolor="#ffcc99">
			...
			<td width="150" align="center" style="border-right:1px solid #333"><strong>Họ tên</strong><strong></strong>
			</td>
		...
		<?php
		$kq = mysqli_query($connection, "select count(*) from thanhvien");
		...
		$kq3 = mysqli_query($connection, $sql3);
		if (!$kq3)
			echo "";
		else {
			while ($r3 = mysqli_fetch_array($kq3)) {
				...
				<tr>
					<td width="50" align="center"
						...
						<input type="checkbox" name="chon[]" value="<?php echo $user3; ?>" />
					</td>
					<td width="150" height="30" align="center"
						style="border-bottom:1px solid #333; border-right:1px solid #333"><b>
							<?php echo "$user3"; ?>
						</b></td>
					...
					<td width="180" align="left"
						style="border-bottom:1px solid #333; border-right:1px solid #333; padding-left:5px; padding-right:5px">
						<?php echo "$diachi"; ?>
					</td>
