...
<table width="1120" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #333">
	<form method="post" name="form">
		...
		while ($r3 = mysqli_fetch_array($kq3)) {
			...
			$soluong = $r3["soluong"];
			$gia = $r3["gia"];
			$gia3 = number_format($gia, 0, '', '.');
			$tong = $gia * $soluong;
			$tong3 = number_format($tong, 0, '', '.');
			$tongtien = $tongtien + $tong;
			$tongtien3 = number_format($tongtien, 0, '', '.');
			$i3++;
			?>
			<tr align="center" height="30">
				...
				<td width="110" style="border-right:1px solid #666; border-bottom:1px solid #666">
					<?php echo $tensp; ?>
				</td>
  ...
  <tr>
    ...
    <input type=\"submit\" name=\"xoagiohang\" value=\"Xóa Giỏ Hàng\" class=\"button2\" onmouseover=\"style.background='url(../../assets/customer/images/button-110-2-o.png)'\" onmouseout=\"style.background='url(../../assets/customer/images/button-110-o.png)'\" onclick=\"document.form.submit();\" /></td>
  </tr>";
		?>
...
<table width="1120" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #333">
	...
	<?php
	$kq = mysqli_query($conn, "select count(*) from giohang,sanpham where giohang.id=sanpham.id AND giohang.user='$user' AND giohang.tinhtrang='damua'");
	...
	if (!$kq2)
		...
		while ($r2 = mysqli_fetch_array($kq2)) {
			...
			<tr align="center" height="30">
				...
				<td align="right" style="border-right:1px solid #666; border-bottom:1px solid #666; padding-right:3px">
					<?php echo $gia2; ?> VND
				</td>
				<td align="right" style="border-right:1px solid #666; border-bottom:1px solid #666; padding-right:3px">
					<?php echo $tong2; ?> VND
