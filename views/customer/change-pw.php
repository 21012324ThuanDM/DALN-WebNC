...
$result_thanhvien = $conn->query("SELECT * FROM thanhvien where user='$idkey'");
$row = $result_thanhvien->fetch_assoc();
$db_user = $row["user"];
?>
<?php
if (isset($_POST["act"])) {
	...
	$result_thanhvien2 = $conn->query($sql);
	$row2 = $result_thanhvien2->fetch_assoc();
	...
	if (strlen($pw) < 6) {
		...
		if ($pw_old == $db_matkhau) {
			$pw = md5($pw);
			$result_changepw = $conn->query("UPDATE thanhvien SET pass='$pww' WHERE user='$user'");
			if ($result_changepw) {
			...
			echo "<script>alert('Đổi mật khẩu thất bại! Kiểm tra lại mật khẩu cũ'); window.history.go(-1);</script>";
		}
	}
}
?>
<form method="post" name="formthanhvien" id="formthanhvien"
	onSubmit="return thanhvien_changepw(pw_old.value,pw.value,cpw.value);">
	<table border="0" width="560" cellpadding="0" cellspacing="0" style="border:1px solid #CCCCCC ; margin-left: 280px">
		<tr class="tieude" align="center" height="35">
			<td colspan="2">ĐỔI MẬT KHẨU</td>
		</tr>
		<tr height="30" bgcolor="#F9F9F9" onmouseover="style.background='#d4340c'"
			onmouseout="style.background='#F9F9F9'">
